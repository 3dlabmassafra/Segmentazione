...
function updateButtons(){for(const s of ['#apply-cuts','#open-file','#upload-zone','#add-plane','#reset-project','#load-demo']) $(s).disabled=state.busy;$('#add-plane').disabled=state.busy||state.planes.length>=5;$('#export-all').disabled=state.busy||state.dirty||!state.parts.length;$$('[data-download]').forEach(b=>b.disabled=state.busy||state.dirty);$$('[data-axis], .plane-slider, .plane-number, [data-remove], [data-mode], [data-preset], #curve-frame, #edit-curve, #point-u, #point-v, #curve-point-select, #exit-curve, #quick-curve-cut').forEach(b=>b.disabled=state.busy||state.drawing);freehand?.syncButtons();}
...
function request(type,data={}){return new Promise((resolve,reject)=>{const id=++taskId;pending.set(id,{resolve,reject});worker.postMessage({id,type,...data},data.positions?[data.positions.buffer]:[]);});}
...
function updateExplosion(){
 const axis=state.appliedAxis;
 group.children.forEach((m,i)=>{
  m.position.set(0,0,0);if(state.editing||state.dirty||state.drawing)return;
  if(state.appliedMode==='freehand'&&state.parts.length>1){
   const b=state.parts[i].bounds,o=state.original.bounds;
   const direction=new THREE.Vector3(...b.min.map((v,a)=>(v+b.max[a]-o.min[a]-o.max[a])/2));
   if(direction.length()<1e-6)direction.set(Math.cos(i*2.4),Math.sin(i*2.4),.1);
   m.position.copy(direction.normalize().multiplyScalar(gap()));
  }else m.position.setComponent(axis,(i-(axis===2?0:(state.parts.length-1)/2))*gap());
 });updatePlanes();
}
function updateXray(){group.children.forEach(m=>{m.material.transparent=state.xray;m.material.opacity=state.xray?.32:1;m.material.depthWrite=!state.xray;});}
function fitView(top=false){if(state.drawing)return;camera.up.set(0,0,1);if(state.editing){updateCurveCamera();return;}if(!state.parts.length)return;const bounds=new THREE.Box3().setFromObject(group);const center=bounds.getCenter(new THREE.Vector3());const size=bounds.getSize(new THREE.Vector3());const vfov=camera.fov*Math.PI/180;const hfov=2*Math.atan(Math.tan(vfov/2)*camera.aspect);const distance=Math.max(size.z/2/Math.tan(vfov/2),Math.max(size.x,size.y)/2/Math.tan(hfov/2),size.length()*.75)*(camera.aspect<.8?1.88:1.6);if(camera.aspect<.8)center.z+=size.z*.025;controls.target.copy(center);if(top){camera.position.copy(center).add(new THREE.Vector3(.001,-.001,distance*1.1));$('#camera-label').textContent='Dall’alto';}else{camera.position.copy(center).add(new THREE.Vector3(1,-1.6,.95).normalize().multiplyScalar(distance));$('#camera-label').textContent='Prospettiva';}camera.near=Math.max(.01,distance/1000);camera.far=distance*50;camera.updateProjectionMatrix();controls.update();}
...
if(triangles&&84+50*triangles===buffer.byteLength){
  if(triangles>2000000)throw new Error('Troppi triangoli');
  positions=new Float32Array(triangles*9);
  for(let i=0;i<triangles;i++)for(let j=0;j<9;j++)positions[9*i+j]=view.getFloat32(84+50*i+12+4*j,true);
}else{const geometry=new STLLoader().parse(buffer);positions=geometry.attributes.position.array;geometry.dispose();}
if(positions.length/9>2000000){toast('Il modello supera il limite di 2.000.000 triangoli. Riduci la mesh e riprova.',true);return;}await loadModel('import',positions,file);}catch(e){toast('Impossibile leggere il file STL. Verifica che non sia danneggiato.',true);}}
...
function syncCurveUI(){
  ...
  $('#straight-controls').hidden=state.mode!=='plane';$('#curve-controls').hidden=!curved;freehand?.sync();
  ...
  $('#plane-count').textContent=state.mode==='freehand'?`${state.freeCuts.length} TAGLI`:curved?'1 CURVA':`${state.planes.length} PIANI`;
  ...
  $('#apply-cuts').innerHTML=`<i data-lucide="scissors"></i> ${state.mode==='freehand'?'Anteprima divisa':curved?'Applica taglio curvo':'Applica tagli'} <kbd>↵</kbd>`;
...
function setupCurveEditor(){
  ...
  $$('[data-mode]').forEach(b=>b.onclick=()=>{
    if(state.busy||state.drawing||state.mode===b.dataset.mode)return;
